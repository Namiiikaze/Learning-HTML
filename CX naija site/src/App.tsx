...
function Home({ go, notify }: { go: (p: Page) => void, notify: (s: string) => void }) {
  ...
  return <>
    ...
    <section data-aos="fade-up" className="spotlight-section section-pad"><div className="spotlight-profile"><img src="https://images.unsplash.com/photo-1600880292203-757bb62b4baf?auto=format&fit=crop&w=900&q=80" alt="Two community members in conversation"/><div><p className="eyebrow"><span></span> Member spotlight</p><h2>“The best CX lesson? <em>Listen for what isn’t being said.”</em></h2><p>Each month, we make room for one member’s journey, their work, and the ideas they are carrying into the profession.</p><button className="text-button" onClick={()=>go('directory')}>Read member stories <span>→</span></button></div></div><div className="wins"><p className="eyebrow"><span></span> Community wins</p><h3>Good news travels here.</h3><ul><li>🎉 Ada spoke at a global conference.</li><li>🎉 A member earned their CCXP certification.</li><li>🎉 A community member stepped into a new leadership role.</li><li>🎉 A customer experience practitioner published their first industry article.</li></ul></div></section>
